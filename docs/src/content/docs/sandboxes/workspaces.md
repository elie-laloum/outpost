...
`label` names generated branches/directories. `hooks.workspaceReady` runs when the workspace opens; other hooks are passed to later sandboxes. `limits` configures Git/copy deadlines. See [hooks](../hooks/) and [WorkspaceOptions](../../reference/workspaceoptions/).
