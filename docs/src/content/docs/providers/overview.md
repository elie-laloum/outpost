...
Docker/Podman default to mounted Git metadata; opt into the [isolated repository prototype](../repository-isolation/) for a private checkout synchronized through bundles. [Firecracker](../firecracker/) is a research backend requiring a prepared Linux host; its simulated tests do not establish a real boot or a production security boundary. [Outbound network policies](../../sandboxes/egress/) support container deny-all and Vercel firewall policies, with provider-specific limits.
