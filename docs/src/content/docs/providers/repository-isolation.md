...
Build the [container image](../agent-images/) first. Omit `networks: "none"` when the task needs network access, including model APIs. Configure authentication using the existing [agent guides](../../agents/connect-codex/); the agent home remains private and ephemeral. The image, engine and explicit credentials remain trusted inputs.
