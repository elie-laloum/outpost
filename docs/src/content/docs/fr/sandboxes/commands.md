...
Docker, Podman et l’exécution locale prennent en charge l’attachement ; Vercel et Daytona le refusent explicitement. `terminal: { input, output, error }` permet de fournir les flux. Le mode brut et la visibilité du curseur sont restaurés à la sortie. Le résultat contient statut, sortie, commits, branche, répertoire et informations de fermeture disponibles.
