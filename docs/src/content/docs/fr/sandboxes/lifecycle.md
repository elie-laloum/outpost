...
Pour un workspace d’intégration, appelez explicitement `sandbox.workspace.integrate()` lorsque le travail doit rejoindre la branche de l’hôte. Relisez et commitez d’abord les changements souhaités. Le [guide des workspaces](../workspaces/) décrit la propriété séparée.
