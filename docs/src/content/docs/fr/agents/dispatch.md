...
`label` apparaît dans le journal et les noms de logs générés. `observe`, `warn` et `diagnostic` apportent de la visibilité ; voir [l’observabilité](../observability/). Le guide [des passes](../iteration/) explique la réutilisation de l’environnement.
