...
Utilisez cette recette pour un défaut bien délimité. Prérequis : [installation commune](../), connexion Claude et projet npm avec lockfile et tests. Adaptez le nom de branche et la description.
...
Un processus ou hook en échec fait échouer le dispatch. Le travail non commité est conservé pour la [récupération](../recovery/). Réutiliser la branche reprend son historique ; choisissez un autre nom pour une tentative indépendante.
