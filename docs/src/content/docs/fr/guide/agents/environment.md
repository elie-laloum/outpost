---
title: "Déclarer uniquement l’environnement nécessaire"
description: "Lisez la configuration choisie sans afficher les valeurs des identifiants."
---

Lisez la configuration choisie sans afficher les valeurs des identifiants.
...
```ts file=example.mts
import { configuration, repository, variables } from "./runtime.mts";
const runtime = await configuration();
console.log({
  agent: runtime.agent.name,
  repository,
  declaredNames: Object.keys(variables),
});
```
