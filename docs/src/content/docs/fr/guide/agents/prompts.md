---
title: "Donner du contexte à l’agent"
description: "Séparez l’objectif du script et insérez le contexte Git actuel dans le brief."
---

Séparez l’objectif du script et insérez le contexte Git actuel dans le brief.
...
const brief = new URL("task.md", import.meta.url);
await writeFile(
  brief,
  "Objective: {{OBJECTIVE}}\nBranch: {{WORK_BRANCH}}\nStatus: !`git status --short`\nRun npm test, commit and print <outpost>done</outpost>.",
);
const result = await dispatch({
  ...runtime,
  branch: { mode: "named", name: "workshop/brief" },
  brief: {
    file: fileURLToPath(brief),
    values: { OBJECTIVE: "Fix whitespace in text.ts" },
  },
  deadlineMs: 300_000,
});
