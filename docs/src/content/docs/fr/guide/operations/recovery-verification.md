...
Utilisez un transfert distant conservé contenant `state.json`. Cette procédure en lecture seule diagnostique sa structure avant toute restauration. Pour reproduire d’abord un workspace local conservé, lancez l’[exemple de récupération](../recovery/).
