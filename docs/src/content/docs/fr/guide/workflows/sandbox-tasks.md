---
title: "Relier un agent à une validation par les tests"
description: "Exécutez une tâche d’implémentation, puis exigez la réussite de la vraie commande de test."
---

Exécutez une tâche d’implémentation, puis exigez la réussite de la vraie commande de test.
...
await using sandbox = await createSandbox({
  ...runtime,
  branch: { mode: "named", name: "workshop/pipeline" },
});
...
const result = await workflow("fix-and-test", [implement, verify]).start();
result.unwrap();
console.log(result.value(verify).status);
```
