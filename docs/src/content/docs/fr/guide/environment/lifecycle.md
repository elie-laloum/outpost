---
title: "Garder un environnement actif"
description: "Créez un fichier temporaire, puis relisez-le dans une seconde commande de la même sandbox."
---

Créez un fichier temporaire, puis relisez-le dans une seconde commande de la même sandbox.
...
const write = await sandbox.command({
  ...
  arguments: [
    "-e",
    "require('node:fs').writeFileSync('/tmp/outpost-note', 'warm')",
  ],
});
assert.equal(write.status, 0);
const read = await sandbox.command({
  executable: "node",
  arguments: [
    "-e",
    "console.log(require('node:fs').readFileSync('/tmp/outpost-note', 'utf8'))",
  ],
});
assert.equal(read.stdout.trim(), "warm");
console.log(read.stdout.trim());
```
