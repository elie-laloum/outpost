---
title: "Comprendre le transfert distant avant modification"
description: "Vérifiez que des données binaires atteignent le système de fichiers actif et que les fichiers temporaires sont nettoyés."
---

Vérifiez que des données binaires atteignent le système de fichiers actif et que les fichiers temporaires sont nettoyés.
...
console.log(report.checks.filter((check) => check.id.includes("transfer")));
if (report.hasFailures)
  throw new Error("Transfer/environment diagnosis failed");
```
...
[Contrats, options et cas particuliers](../../../reference/behavior/sandboxes/remote-sync/).
