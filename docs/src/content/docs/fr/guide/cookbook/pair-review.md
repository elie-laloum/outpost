---
title: "Faire implémenter Codex et relire Claude"
description: "Transmettez les mêmes fichiers d’un agent à l’autre, puis vérifiez leur état final par les tests."
---

Transmettez les mêmes fichiers d’un agent à l’autre, puis vérifiez leur état final par les tests.
...
Déclarez **à la fois** `OPENAI_API_KEY=` et `ANTHROPIC_API_KEY=` dans `workflow/.env` pour cette recette à deux agents et clés API. Le provider reçoit les deux valeurs déclarées. Les agents partagent fichiers et historique Git, mais pas les conversations natives. Les dépendances sérialisent les opérations. La revue peut modifier des fichiers, donc les tests passent ensuite. Rien ne fusionne automatiquement. Pour une version séquentielle, utilisez deux `sandbox.dispatch()` dans cet ordre et vérifiez le statut de `sandbox.command()`.
