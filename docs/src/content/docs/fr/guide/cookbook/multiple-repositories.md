---
title: "Coordonner deux dépôts"
description: "Transmettez le résultat du premier dépôt à une tâche qui travaille sur un autre dépôt."
---

Transmettez le résultat du premier dépôt à une tâche qui travaille sur un autre dépôt.
...
Avant l’exécution, revenez dans `outpost-example/` et lancez `node prepare.mjs repository-two`, puis retournez dans `workflow/`. Chaque tâche possède son dépôt, sa branche, ses logs et ses ressources. La dépendance transmet explicitement du texte ; elle ne copie ni commits ni fichiers. Si la seconde tâche échoue, les commits du premier dépôt restent. Aucune transaction commune ni push automatique ne couvre les deux.
