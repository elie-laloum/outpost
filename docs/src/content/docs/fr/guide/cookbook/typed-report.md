---
title: "Renvoyer un rapport de défaut validé"
description: "Demandez à un agent d’examiner le projet et transformez sa réponse en données exploitables."
---

Demandez à un agent d’examiner le projet et transformez sa réponse en données exploitables.
...
const result = await dispatch({
  ...runtime,
  branch: { mode: "named", name: "workshop/report" },
  brief: {
    text: 'Inspect text.ts without editing it. Return <report>{"summary":"your finding"}</report>.',
  },
  deadlineMs: 300_000,
  response: response.json({
    ...
    schema(value) {
      if (
        ...
        throw new Error("Expected a summary string");
