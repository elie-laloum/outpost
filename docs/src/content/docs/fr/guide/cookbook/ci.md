---
title: "Exécuter une tâche d’agent bornée en CI"
description: "Exécutez le même projet avec un job GitHub Actions manuel et exportez des patches à relire."
---

Exécutez le même projet avec un job GitHub Actions manuel et exportez des patches à relire.
...
Vérifiez d’abord le script localement. Dans un nouveau dépôt GitHub de démonstration, commitez `prepare.mjs`, `workflow/runtime.mts` et `workflow/example.mts` fournis ici. Ajoutez le YAML sous `.github/workflows/outpost.yml` et configurez un secret Actions `OPENAI_API_KEY`. Ne commitez ni `.env` ni artefacts d’exécution. Le job crée son dépôt ciblé et exporte les patches commités ; il ne pousse rien et ne publie aucune pull request. Le statut CI prouve la fin du script, pas la justesse du patch : relisez et testez l’artefact.
...
jobs:
  fix:
    ...
    steps:
      ...
      - run: npx @elie-laloum/outpost init --yes --directory workflow --repository ../repository --image outpost:docs-demo --install
      - name: Configure the selected agent
        working-directory: workflow
        run: printf 'OUTPOST_AGENT=codex\nOUTPOST_AUTH=api-key\nOPENAI_API_KEY=\n' > .env
      - name: Run the task
        working-directory: workflow
        run: node example.mts
        env:
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
      - name: Export committed changes
        run: git -C repository format-patch main..workshop/fix --output-directory ../patches
