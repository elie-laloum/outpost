---
title: "Comparer des corrections candidates validées"
description: "Mettez en concurrence deux approches bornées du même défaut et sélectionnez un candidat après les tests."
---

Mettez en concurrence deux approches bornées du même défaut et sélectionnez un candidat après les tests.
...
const result = await speculate({
  ...
  sandbox: { hooks: runtime.hooks },
  concurrency: 2,
  budget: { attempts: 2 },
  candidates: ["minimal", "alternative"].map((key) => ({
    key,
    agent: runtime.agent,
    request: {
      brief: {
        text: `Use a ${key} approach to fix whitespace in text.ts. Test and commit.`,
      },
      deadlineMs: 300_000,
    },
  })),
