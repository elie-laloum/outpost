---
title: "Envelopper un provider via son contrat"
description: "Ajoutez une observation d’allocation en déléguant exécution et nettoyage à un backend existant."
---

Ajoutez une observation d’allocation en déléguant exécution et nettoyage à un backend existant.
...
const instrumented = mountedProvider({
  ...
  async acquire(context) {
    ...
    return backend.acquire(context);
  },
});
...
console.log("provider command completed");
```
