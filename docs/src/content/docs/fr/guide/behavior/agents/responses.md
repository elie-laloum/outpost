...
`repairs` est un entier positif ou nul, égal à `0` par défaut. Une réparation reprend la même conversation avec le retour de validation ; l’adapter doit donc permettre la reprise. Claude Code et Codex le permettent ; [Gemini](../../../agents/gemini/) exige `repairs: 0`. Les réponses structurées exigent `passes: 1`.
