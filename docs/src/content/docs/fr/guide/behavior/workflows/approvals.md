...
Utilisez `approvalTask()` pour une étape approuver/refuser et `pauseTask()` pour une étape reprendre/refuser. Ce sont des dépendances ordinaires du graphe reposant sur un [checkpoint](../../../advanced/checkpoints/). Lorsque l'étape est prête, Outpost persiste sa demande et marque la tâche `paused`. Les tâches indépendantes se terminent normalement ; les dépendantes restent en attente. `start()` renvoie le statut `paused` après la fin des opérations actives et libère le verrou du checkpoint. Aucun minuteur ni processus actif n'est nécessaire pour conserver la demande.
