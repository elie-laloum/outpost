...
Construisez et authentifiez l’image selon la [configuration du fournisseur](../../../environment/providers/overview/). Cette fonction ne configure pas les identifiants de l’agent. Le validateur obligatoire intervient après un dispatch réussi, avec la sandbox encore ouverte. Retournez `true` seulement si la sortie satisfait vos critères. Une exception de validation fait échouer ce candidat. Limitez les modifications du validateur aux sorties temporaires des tests ; les commits retournés décrivent le dispatch, avant validation.
