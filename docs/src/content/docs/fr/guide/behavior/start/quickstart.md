...
Pour un accès par compte, suivez [Connecter Codex](../../../agents/connect-codex/) ou [Connecter Claude](../../../agents/connect-claude/). La CLI inclut déjà le hook de connexion par clé API Codex dans les scripts générés ; une composition manuelle avec l’API doit le configurer explicitement.
