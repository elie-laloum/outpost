...
Utilisez `agent({ harness: codex.harness(), model: "..." })`, ou les presets `claude.harness()` et `gemini.harness()`. `customHarness({ modelProvider, run })` relie un callback de l’appelant à un service de modèles. `sandboxProvider` choisit indépendamment où exécuter les commandes du dépôt.
