...
`local()` exécute les commandes directement avec votre compte système. Il convient à l’automatisation locale de confiance ou aux tests de contrat sans moteur de containers.
...
await dispatch({
  agent: codex(),
  provider: local({ variables: { PROJECT_MODE: "test" } }),
  branch: { mode: "named", name: "local/inspection" },
  brief: { text: "Examine le dépôt et résume tes observations." },
});
```

Installez et authentifiez d’abord le CLI sur l’hôte. `local()` accepte les `variables` du provider ; il ne construit pas d’image et ne crée aucun service externe. Les commandes utilisent le répertoire du workspace par défaut. Environnement et accès aux fichiers suivent votre compte système.
