...
Le rapport conserve aussi `startedAt`, la version de Node.js et `source.commit`/`source.dirty` depuis la copie des sources de la fixture avant allocation. Placez la sortie hors du dépôt pour que sa création ne marque pas les sources comme modifiées. Une copie modifiée ne permet pas de prouver la compatibilité du seul commit indiqué ; les métadonnées Git indisponibles valent `null`. Ces champs sont des observations locales, pas une provenance signée. Archivez ensemble l'URL du workflow et son rapport, puis comparez le commit indiqué à celui que vous souhaitez valider. Une réussite sur un ancien commit distant ne valide pas des changements non poussés.
...
Pour sonder une sandbox que vous possédez déjà, consultez les [diagnostics](../../../../guide/operations/doctor/).
