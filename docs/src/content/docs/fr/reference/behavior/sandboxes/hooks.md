...
Chaque commande accepte `directory`, `variables`, `deadlineMs` et `signal`. Les hooks de sandbox peuvent demander `elevated: true` lorsque le provider le permet. Un échec d’initialisation conserve le travail modifié et retire les workspaces propres possédés. Voir [la récupération](../../../../guide/operations/recovery/).
