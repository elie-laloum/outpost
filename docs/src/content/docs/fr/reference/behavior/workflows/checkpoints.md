...
Consultez les [politiques d'exécution](../../../../guide/workflows/execution/) et les [budgets d'usage](../../../../guide/workflows/budgets/) pour l'annulation, les retries et les limites d'admission.

Un `DispatchResult` complet contient des fonctions de continuation et ne peut pas être sauvegardé directement. Appelez `dispatch` dans une tâche, transmettez son signal et déclarez sa consommation, puis retournez des données simples comme `result.value`. Une tâche d’artefact peut publier ces données et retourner une référence JSON ; voir les [artefacts typés](../../../../guide/advanced/artifacts/).
