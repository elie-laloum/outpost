...
Le `dispatch` de premier niveau exige un agent et un brief. Il combine la [configuration de sandbox](../../../sandboxoptions/) et les [options de dispatch](../../../dispatchoptions/). Il ferme les ressources créées ; un workspace fourni reste ouvert.

`repository` sélectionne le dépôt Git local à modifier. Sans cette option, la bibliothèque utilise `process.cwd()`. Pour un chemin indépendant du dossier de lancement, utilisez `resolve(import.meta.dirname, "../backend")` ; voir [choisir un dépôt](../../../../guide/environment/repositories/).
...
`label` apparaît dans le journal et les noms de logs générés. `observe`, `warn` et `diagnostic` apportent de la visibilité ; voir [l’observabilité](../../../../guide/agents/observability/). Le guide [des passes](../../../../guide/agents/iteration/) explique la réutilisation de l’environnement.
