...
| `text-delta`     | `((event: { readonly kind: "text-delta"; readonly text: string; } & { readonly pass: number; readonly at: string; }) => void \| Promise<void>) \| undefined`                                                                                                             | Optionnel | Traite : fragment de texte du modèle diffusé en streaming par un harness personnalisé ; reçoit pass et at et peut retourner une promesse.                               |
