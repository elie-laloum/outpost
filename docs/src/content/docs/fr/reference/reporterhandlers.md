...
| `tool`         | `((event: { readonly kind: "tool"; readonly name: string; readonly input: unknown; } & { readonly pass: number; readonly at: string; }) => void \| Promise<void>) \| undefined`                                                                     | Optionnel | Traite : nom et entrée d’outil signalés par l’agent ; reçoit pass et at et peut retourner une promesse.                                      |
