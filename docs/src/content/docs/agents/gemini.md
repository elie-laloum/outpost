...
`gemini()` runs the native Gemini CLI through the same sandbox providers as the other adapters. Generated Docker/Podman recipes install Gemini alongside Claude Code and Codex; remote providers bootstrap the pinned CLI when needed unless `bootstrap: false`. For `local()`, install `@google/gemini-cli` yourself; this provider runs directly on the host.
...
A Gemini API key uses the Gemini API account's quotas and billing. Google account login is a separate CLI authentication method with its own eligibility and limits; a Google AI subscription does not turn an API key into subscription authentication. For local account login, run `gemini` interactively and follow its browser flow. `local()` can use host login state; a container has its own ephemeral private home and does not inherit that state. Outpost does not relocate Gemini login credentials or persist a partial home. Follow the current [Gemini authentication instructions](https://geminicli.com/docs/get-started/authentication/) when selecting Google login, an API key or Vertex AI.
...
Build the generated image before this example. `model` accepts a Gemini CLI model name or alias. Headless commands send the prompt through stdin and request `stream-json`. The default headless `approvalMode` is `yolo`, allowing tools to run unattended inside the selected environment. Set `approvalMode` explicitly to `default`, `auto_edit` or `plan` when appropriate; approvals that require terminal input are unsuitable for unattended work. Outpost's provider supplies isolation; `local()` remains unisolated.

Interactive sessions use the CLI terminal interface and default to `approvalMode: "default"`. They require a provider with interactive terminal support, such as Docker, Podman or local execution. An initial prompt uses `--prompt-interactive`. See [interactive sessions](../../sandboxes/commands/) and the official [CLI options](https://geminicli.com/docs/cli/cli-reference/).
