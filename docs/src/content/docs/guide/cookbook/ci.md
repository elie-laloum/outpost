---
title: "Run a bounded agent job in CI"
description: "Run the same workshop from a manual GitHub Actions job and export patches for review."
---

Run the same workshop from a manual GitHub Actions job and export patches for review.
...
Verify the script locally first. In a new demonstration GitHub repository, commit `prepare.mjs`, `workflow/runtime.mts` and `workflow/example.mts` from this page. Add the YAML below as `.github/workflows/outpost.yml` and configure an `OPENAI_API_KEY` Actions secret. Do not commit `.env` or runtime artifacts. The job creates its own target repository and uploads committed patches; it does not push or publish a pull request. A CI exit code proves script completion, not patch correctness: review and test the artifact.
