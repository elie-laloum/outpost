---
title: "Implement, review and test sequentially"
description: "Have Codex fix the defect, Claude review it, and the program verify the tests."
---

Have Codex fix the defect, Claude review it, and the program verify the tests.
...
Declare `OPENAI_API_KEY=` and `ANTHROPIC_API_KEY=` in `workflow/.env`. This recipe uses API keys for both agents; both declared values are forwarded to the container.
...
The two calls are awaited in order. An error stops subsequent steps. The program actually runs `npm test` after review and requires exit status zero; asking an agent to test does not replace this check. Agents share files, not conversations. The sandbox closes on exit; the named branch and any uncommitted work remain recoverable. There is no automatic merge or push. Start again in a new demonstration directory.

[The same recipe as a workflow](../pair-review/).
