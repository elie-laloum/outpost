...
`local()` runs commands directly as your OS account. It is useful for trusted local automation or contract testing without a container engine.
...
Install and authenticate the agent CLI on the host first. `local()` accepts provider `variables`; it does not build an image or provision an external service. Command directories default to the workspace path. Environment and filesystem access follow your host account.
