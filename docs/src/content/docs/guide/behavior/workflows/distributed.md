...
The optional [BullMQ/Redis adapter](../../../advanced/bullmq/) implements the same `TaskQueue` without an HTTP coordinator. It uses Redis time for leases and retains Outpost state alongside BullMQ jobs. Close its connections with `await queue.close()`. HTTP limits and synchronous SQLite closure do not apply to this adapter.
