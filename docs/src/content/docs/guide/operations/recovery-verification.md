...
Start by [inspecting retained data](../recovery/).

Use an existing retained remote transfer containing `state.json`. This read-only procedure diagnoses its structure before any restoration. To reproduce a retained local workspace first, run the [recovery example](../recovery/).
