...
Choose authentication explicitly with `outpost init --agent … --authentication …`. The generated script prepares the selected authentication; constructing an adapter alone does not sign in. Start with the [complete first run](../../start/quickstart/) or inspect the [CLI contract](../cli/).
