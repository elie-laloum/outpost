...
Use this for one well-defined defect. Prerequisites: [shared setup](../), Claude authentication and an npm project with a lockfile/test script. Adapt the branch name and defect description.
...
A failed process or hook rejects the dispatch. Dirty work is retained for [recovery](../recovery/). Reusing the branch continues its history; choose another name for an independent attempt.
