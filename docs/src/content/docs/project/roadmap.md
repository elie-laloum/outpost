...
The working tree implements `agent({ harness, model })`, CLI harness presets, caller-supplied `harness({ modelProvider, run })` callbacks, `openaiModelProvider` and `anthropicModelProvider`. These are unreleased breaking changes; the published 4.2.0 API remains unchanged. Models are names or `{ name, reasoning, maxOutputTokens }` objects validated by the executing harness or provider, without a local availability catalog.
