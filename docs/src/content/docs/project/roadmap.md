...
## Built-in harness engine: remaining validation

Since 5.0.0, `harness()` runs the model loop in the Outpost process. Providers exchange messages, tool calls, replayable reasoning and streamed text with Chat Completions, Responses and Anthropic Messages, including history caching. Tools run through the borrowed sandbox with validation, bounded concurrency, per-call deadlines and step, tool-call and token limits; hooks and declarative permissions control them. Built-in toolsets cover files, edits, search, Git and shell commands. Append-only transcripts support continuation, fork and response repairs, and context strategies compact long histories. Skills add instructions and tools on demand.

These APIs remain experimental. Their tests use scripted providers, local simulated HTTP services and the local sandbox provider. Before broader compatibility claims, the engine needs authenticated campaigns against Anthropic and OpenAI (tool use, reasoning replay, caching and streaming), built-in toolset runs in Docker, Podman and remote sandboxes, and complete repository-editing scenarios compared with the CLI harnesses. Interactive attachment and sub-agents for custom harnesses remain future work. No release date is promised.
