...
Reuse one sandbox across implementation and review. Declare **OPENAI_API_KEY=** and one Claude credential in **.outpost/.env**, and supply both values through your environment. This uses API-key login for Codex; the [account-cache hook](../../../../guide/agents/connect-codex/) is an alternative.
...
The test command supplies a real exit status. The review is advisory; nothing merges automatically. Inspect the named branch or use the [delivery gate](../../../../guide/cookbook/delivery-gate/). Scope exit closes the sandbox even when a test fails.
