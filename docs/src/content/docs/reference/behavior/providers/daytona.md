...
`connection` configures the SDK client. `create` accepts the SDK’s image or snapshot creation parameters. Outpost also accepts `root`, `variables` and `retain`. Model credentials must be supplied independently through [agent environment configuration](../../../../guide/agents/environment/).
...
Read [remote synchronization](../../../../guide/environment/remote-sync/) before enabling `includeUncommitted` or editing the host workspace concurrently. Provider contract tests do not provision a live paid sandbox.
