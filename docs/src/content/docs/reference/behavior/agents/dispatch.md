...
A top-level `dispatch` requires an agent and brief. Its options combine [sandbox configuration](../../../sandboxoptions/) and [dispatch configuration](../../../dispatchoptions/). It closes resources it created; a supplied workspace remains open.

`repository` selects the local Git repository to change. Without it, the library uses `process.cwd()`. For a path independent of the launch directory, use `resolve(import.meta.dirname, "../backend")`; see [choose a repository](../../../../guide/environment/repositories/).
...
`label` appears in journal records and generated log names. `observe`, `warn` and `diagnostic` add visibility; see [observability](../../../../guide/agents/observability/). [Multi-pass execution](../../../../guide/agents/iteration/) explains environment reuse.
