...
Interactive sessions use the CLI terminal interface and default to `approvalMode: "default"`. They require a provider with interactive terminal support, such as Docker, Podman or local execution. An initial prompt uses `--prompt-interactive`. See [interactive sessions](../../../../guide/environment/commands/) and the official [CLI options](https://geminicli.com/docs/cli/cli-reference/).
