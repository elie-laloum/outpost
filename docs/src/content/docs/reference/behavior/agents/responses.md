...
`repairs` is a nonnegative integer, default `0`. A repair resumes the same conversation with validation feedback, so the adapter must support resumption. Claude Code and Codex support it; [Gemini](../../../../guide/agents/gemini/) requires `repairs: 0`. Structured responses require `passes: 1`.
