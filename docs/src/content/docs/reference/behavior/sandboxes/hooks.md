...
Each command may supply `directory`, `variables`, `deadlineMs` and `signal`. Sandbox hooks may request `elevated: true` where supported. A failed initialization preserves dirty work and removes clean owned workspaces. See [recovery](../../../../guide/operations/recovery/).
