...
Uncommitted changes are preserved in the workspace; they are not magically converted into commits. A merge conflict retains the managed workspace for inspection. Inspect `retainedDirectory` or [recovery metadata](../../../../guide/operations/recovery/) before cleanup.
