...
See [choose a repository](../../../../guide/environment/repositories/) for workflow directories, target repositories and relative paths.
...
Set `MODEL_API_KEY` in the parent environment or workflow `.env` before running. `--api-key-env` defaults to `OPENAI_API_KEY` and requires `--base-url`; the custom endpoint requires `--model`. This path uses the custom provider directly without OpenAI account login. Chat Completions-only endpoints are unsupported. See [custom model providers](../../agents/connect-codex/#openai-compatible-model-providers).
