...
- Accepte `agent({ model })` sous forme de nom ou d’objet `AgentModel` avec `reasoning` et `maxOutputTokens`. `reasoning` quitte `claudeHarness()` et `codexHarness()`, et `maxOutputTokens` quitte `anthropicModelProvider()`. Le harness CLI ou le fournisseur de modèles refuse les réglages non pris en charge à la composition de l’agent : Claude Code les traduit en `--effort` et `CLAUDE_CODE_MAX_OUTPUT_TOKENS`, Codex n’accepte que le raisonnement, Gemini CLI n’accepte ni l’un ni l’autre, Anthropic exige une limite de sortie et OpenAI transmet l’effort de raisonnement. `agent.model` devient un objet normalisé. Ces changements sont cassants.
- Étend le contrat modèle expérimental avec `messages`, la déclaration d’outils, les blocs `tool-call`/`tool-result`, des blocs de raisonnement opaques rejoués uniquement à l’identité de fournisseur et au modèle d’origine, une `stopReason` normalisée et une option de cache d’historique pour Anthropic. Les trois protocoles traduisent les appels d’outils sans les exécuter, et les réponses tronquées ou refusées sont signalées au lieu d’être rejetées.
- Remplace les callbacks `harness({ modelProvider, run })` par un moteur intégré déclaratif : `harness({ modelProvider, instructions, tools, limits, toolExecution, cache })`. Ajoute `defineHarnessTool()` (entrées en sous-ensemble JSON Schema ou Standard Schema), `defineHarnessToolset()` et `defineHarnessInstructions()`. Le moteur exécute les outils via le sandbox emprunté avec validation des entrées, concurrence des outils en lecture seule, délai par appel et résultats ordonnés, renvoie les erreurs d’outils au modèle et échoue avec le nouveau code d’erreur `limit` sur les limites d’étapes, d’appels d’outils, de tokens ou de sortie. Les observateurs du dispatch reçoivent les événements `step` et `tool-result`. `run`, `HarnessInput`, `HarnessContext` et `HarnessRun` sont supprimés. Hooks, permissions, jeux d’outils fournis, conversations personnalisées persistées et streaming restent prévus.
- Ajoute `defineHarnessHook()` pour les points de contrôle `session-start`, `before-model`, `after-model`, `before-tool`, `after-tool` et `stop`, et `defineHarnessPermissions()` pour des règles ordonnées d’autorisation et de refus sur les noms d’outils, les commandes et les chemins du dépôt déclarés par `resources()`. Les permissions s’exécutent avant les hooks et de nouveau après la réécriture d’un appel par un hook ; les refus et les arrêts refusés sont signalés au modèle et émis sous forme d’événements `tool-denied` et `stop-prevented`. Une exception d’un hook fait échouer la passe.
- Ajoute des jeux d’outils fournis pour le harness : `harnessFileTools()` (`read_file`, `list_files`), `harnessEditTools()` (`write_file`, `edit_file` par remplacement exact avec détection de conflit), `harnessSearchTools()` (`git grep`), `harnessGitTools()` (`status`, `diff`, `log`, `show` en lecture seule) et `harnessShellTools()` (`sh -c` avec délai). Le contenu des fichiers circule par des transferts qui préservent les données binaires, les chemins restent dans le dépôt, et chaque outil déclare ses ressources pour les règles de permission.
- Enregistre les passes des harness personnalisés dans des transcriptions en ajout seul sous `.outpost/conversations/harness/`, exclues de Git, avec `harnessConversations()` comme store par défaut et `transportConversations("harness", …)` pour le stockage distant. Les harness personnalisés prennent désormais en charge continuation, fork et réparations de réponse (outils en lecture seule pendant les réparations), tiennent un verrou par conversation et ferment les appels d’outils interrompus par des résultats en erreur à la reprise. Ajoute `defineHarnessContextStrategy()`, `truncateToolResults()` et `summarizeHistory()` ; les compactions sont enregistrées dans la transcription, retirent le raisonnement rejoué et émettent des événements `compaction`. `conversations: false` désactive l’enregistrement.
- Ajoute `defineHarnessSkill()` pour des instructions et des outils qu’un harness personnalisé charge à la demande. Les skills sont listées dans les instructions système et chargées via un outil réservé `load_skill` ; leurs outils sont déclarés dès le départ mais refusés tant que la skill n’est pas chargée, ce qui garde stables la liste d’outils et le cache du fournisseur. Les skills chargées sont relues depuis la conversation.
- Ajoute `ModelProvider.stream()`, optionnel, aux trois fournisseurs intégrés : les événements server-sent sont décodés en fragments `text-delta` et en un même `ModelResult` final, `timeoutMs` mesurant l’inactivité entre deux fragments. Les harness personnalisés utilisent le streaming automatiquement et émettent des événements `text-delta`, que les journaux ne gardent qu’en mode verbeux et que le rapporteur du terminal n’affiche qu’une fois.
