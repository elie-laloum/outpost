...
Compose agents with `agent({ harness, model })`; `codexHarness()`, `claudeHarness()` and `geminiHarness()` are CLI presets. `harness({ modelProvider, run })` supplies an application callback, not a built-in tool loop. Models are names or `{ name, reasoning, maxOutputTokens }` objects; the executing harness or model provider validates them when the agent is composed and rejects unsupported settings. Use `AgentAdapter` only for CLI protocol behavior, `SandboxProvider`/`SandboxLease` and explicit `*SandboxProvider()` factories for execution environments and `ConversationStore` for transcript persistence. Prefer composition and injected capabilities to inheritance or branching on provider names throughout the application. Extend the relevant adapter or strategy when introducing a variant.
